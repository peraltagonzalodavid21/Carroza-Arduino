...
// (Evitamos pines de booteo como el 0, 2, 5, 12 y 15 para evitar problemas al
// encender)
int pinesRelevadores[] = {13, 14, 25, 26, 27, 32, 33, 16, 17, 18, 19, 21};

void setup() {
  // Iniciamos el puerto Serial normal para ver mensajes en la compu (monitor
  // serie)
  Serial.begin(115200);
  ...
  Serial.println(
      "¡El Bluetooth del ESP32 está listo! Vinculalo desde el celular.");
}
...
void loop() {
  ...
  if (SerialBT.available() > 0) {
    ...
    // Imprimimos la letra recibida en la PC para que veas que está llegando
    // bien
